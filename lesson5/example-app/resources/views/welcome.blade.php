...
@php
   ...
   /* use App\Models\User;
    ...
    dd($user);*/
  //dd($users->map(function($user){return $user->id;})->avg());
  /*dd($users->avg('id'));*/
  //dd($users->contains('email','hmertz@example.org'));
  //dd($users->containsStrict('id',1));
  //dd($a->countBy());
  /*$filter = $users->filter(function ($val, $key){
      if($val['email'] =='hmertz@example.org')
          return true;
      return false;
  });
  dd($filter);*/
  /*$user = $users->search(function ($val, $key){
      return $val['email']=='hmertz@example.org';
  });*/
  /*$user = $users->get('email')->search(('hmertz@example.org');
  dd($user);*/

/*$arr = collect( [
    ['id'=>1, 'email'=>'hmertz@example.org', 'name'=>'John'],
    ['id'=>2, 'email'=>'hme@example.org', 'name'=>'Done'],
    ['id'=>3, 'email'=>'rtz@example.org', 'name'=>'Hon'],
    ]);
dd($arr->groupBy('email'));*/

/*$emails = $users->pluck('email');
$em = $emails->forget(1);
dd($em)->toArray();*/

/*$sortBy = $users->sortBy('email');
dd($sortBy);*/


/*$a = collect([1,24,2,14,3,35, 4,5,6]);

$skip = $a->skip(4);
dd($skip);*/

/*$arr = collect(['Kill', 'Lola', 'Pol', 'John']);
dd($arr->replace([1=>'Den', 3=>'Rocky']));*/

/*$arr = collect(['Welcome', 'to', 'Home', 'Bill']);
dd($arr->join(' '));*/

//dd($users->sortBy('name')->implode('name', ', '));
